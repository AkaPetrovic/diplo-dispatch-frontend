...
const DeleteTruckLoadPage = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8080/api/loads/delete", {
        method: "DELETE",
        headers: {
  ...
  const handleInitiateConfirmDialog = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setDialogModalMessage(
      "Are you sure that you want to delete the selected truck load from the database?",
    );
    setIsDialogOpen(true);
  };
    ...
    <main className="flex h-full w-full flex-col items-center justify-center">
      <div className="w-1/4 min-w-fit px-8 py-5 shadow">
        <div className="flex flex-col">
          <h1 className="mb-2">Delete truck load</h1>
        </div>
